...
перед отправкой проекта (бэк и фронт) на сервер reg.ru необходимо сделать сборку фронт-проекта на локальной машине.
...
, исключить папку build из списка в .gitignore, отправить на виртуальный сервер при отправке репозитория проекта.

2 вариант:

осуществить сборку проекта на сервере reg.ru, предварительно установив все необходимые пакеты

```bash
sudo apt install nodejs
```

из папки frontend на сервере:
...
или
создаем отдельный файл, если используем переменные окружения в нем, а все данные прописываем в settings через os.getenv
...
![gunicornservice](./readme_img/gunicornservice.jpg)
![gunicornsocket](./readme_img/gunicornsocket.jpg)
...
Проверяем наличие сборки папки build на сервере.

## nginx

Теперь перейдем к настройкам nginx.  
Создадим новый файл, можно без расширения, имя выбираем самостоятельно (например, mycloud):
...
![nginx_sites_available](./readme_img/nginx_sites_available.jpg)
![nginx_sites_available2](./readme_img/nginx_sites_available2.jpg)
Чтение лекций и поиск по сети, к сожалению, не дали рабочей формулы того, как приживить на одном сервере react и django.
Методом проб и ошибок был выбран вариант настроек выше. Увы, в таком случае требуется для посещения сайта дописывать порт (:3000). Попытка разместить front на 80 порту привела к тому, что открывается толко главная страница, а на api запросы сервер отвечает ошибкой 403.

Cоздаем символическую ссылку на эти файлы в другую директорию:
